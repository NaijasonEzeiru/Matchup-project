...
const PetDetails = ({ params }: { params: { id: string } }) => {
  const [pet, setPet] = useState<PetSchemaType>();
  ...
  const getPets = async () => {
    const res = await fetch(`${apiAddress}/api/pets/${params.id}`);
  ...
  return (
    <main className='min-h-[calc(100vh-30rem)] md:min-h-[calc(100vh-15rem)]'>
      ...
      <form className='py-16 px-3 md:px-14 lg:px-32 flex flex-col gap-3'>
